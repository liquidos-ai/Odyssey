...
The TUI runs through the main CLI entrypoint:
...
- plain text sends a normal chat turn to the active session
- slash commands such as `/new` manage sessions and bundle state
- lines that start with `!` run a direct sandboxed process inside the current session, for example
  `!ls -la`
